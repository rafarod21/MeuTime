...
export function translateCountryNameForCode(code: string) {
...
export function translateCountryNameForName(name: string) {
  const country = ISO_COUNTRIES.find((country) => country.cname === name);

  if (country) return translateCountryNameForCode(country.ccode);
